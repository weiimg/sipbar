...
| 腳本 | 內容 |
|---|---|
| `test_flow.py` | **端對端**：第一次啟動 → 引導 → 提醒升級出聲 → 補水 → 退回 → 達標 → 換日 → 改設定 → 重看導覽。其他幾支各驗一個零件，這一支驗零件之間的接縫 |
| `test_island.py` | 島的 28 組：提醒時機、閒置不計時、升級、達標、換日、暫停、跨重啟接續、探頭、遮罩與視窗尺寸、文字寬度、延遲動畫的取消、防連點、**升級才出聲**（啟動接回狀態不能響）、**退回一次記錄**（連倒數一起還原，否則變成隱藏的關閉鍵） |
| `test_streak.py` | 連續天數 11 組：累積、拍攝日跳過、護盾消耗、跨月重置、逐日遞增 |
| `test_spring.py` | 彈簧 8 組：過衝幅度、收斂時間、被打斷時的連續性、掉幀保護 |
| `test_sound.py` | 提示音：格式、**音量上限**、音檔是不是工具產的那一份、播放旗標、每一條失敗路徑都要沉默、**自訂音效**（選檔驗格式、驗不過不覆蓋、還原不刪原檔）、引導有沒有告知 |
| `test_crashlog.py` | 閃退紀錄：寫入、上限截斷、路徑異常時不能自己炸掉 |
| `test_settings.py` | 設定：體重換算、作息推導、舊鍵升級、設定檔遷移、齒輪點擊區、**改設定不重置倒數**、**捲動區不能是透明的洞**、換主題要連外框一起換、護盾的補滿說明、**引導不能悄悄改掉現有設定** |
