...
所以看起來像殘留檔案的問題。2026-08-20 發 0.10.1 時卡在這裡三次才找到原因。

### 同一個訊息有兩種原因，解法相反（2026-08-24 補）

上面那段只寫了其中一種。實際上 WinError 5 有兩個來源：

| 原因 | 症狀 | 解法 |
|---|---|---|
| **殘留的資料夾帶 ReadOnly 屬性** | **每一次都卡在同一個路徑**，重跑幾次都一樣 | 刪之前先清屬性。`build_exe.py` 的 `_force_rmtree()` 已經自動做了 |
| 真的有東西抓著（Sipbar 在跑、防毒在掃） | 每次卡在不同路徑，隔幾秒手動刪得掉 | 關掉 Sipbar；`build_exe.py` 會自動重試三次 |

`os.rmdir()` 對帶 ReadOnly 屬性的資料夾一律回 WinError 5，跟「被佔著」的訊息
一模一樣。**分辨方法**：PowerShell 的 `Remove-Item -Force` 刪得掉就是第一種
（`-Force` 會先清屬性），刪不掉才是第二種。

`shutil.rmtree(ignore_errors=True)` 在這裡是陷阱：它把唯讀那一種安靜地跳過，
於是殘留留在原地、下一輪撞同一個地方，而畫面上看起來像「清過了還是失敗」。

2026-08-24 這次花了四輪才分清楚：先照上面那段找有沒有 Sipbar 在跑（沒有），
再猜防毒（加了重試，還是每次卡在同一個 `localpycs`），最後才去看那個資料夾的
屬性。**兩種原因現在都自動處理掉了**，正常情況不會再看到這個錯誤；真的再看到，
代表是第二種而且連重試都救不了。
...
**寫法照 [CHANGELOG.md](CHANGELOG.md) 開頭那幾條規則**（不用破折號、不寫設計
理由、不要 AI 口語、章節標題用名詞）。那幾條在這裡同樣適用，而且更容易漏：
release notes 是發版當下趕出來的，0.10.1 那則就漏了一個全形破折號進去。

**結構用 Smart Brevity**：精簡但不省略。每一個功能拆成帶粗體標籤的短段，
不要寫成散文。

```
## 新增版本更新通知

**為什麼重要**：先前新版發布沒有任何通知，要自己去 Releases 頁面看。
...
「精簡」是指刪掉不做事的字，**不是刪掉資訊**。上面那幾個標籤加起來要涵蓋
「改了什麼、為什麼有差、怎麼用、有什麼不做、怎麼關掉」。
