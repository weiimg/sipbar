...
# ---------------------------------------------------------------- 文案
#
# 語氣參考任天堂的 CF 與實機遊玩影片：**平實地描述「會發生什麼」，
# 不推銷、不加形容詞、不用比喻。**「按這個按鈕，就會跳。」可愛來自事情本身，
# 不來自語氣。第三頁本來就是實機影片（上面那個動畫預覽），語調正好對得上。
#
# **關鍵是人稱。** 引導用第三人稱描述那隻杯子，杯子自己不開口；島的台詞
# （「口渴了」「你贏了」）維持第一人稱。兩者形成「說明書 vs 角色」的對比，
# 而且**不開口的角色沒辦法情緒勒索**。
#
# 前面走過兩版都不對，值得記著：
#
...
# - 第二版學 Duolingo，寫成「我不怪你，但我也不會閉嘴」。那是**威脅包成體諒**：
#   先替使用者預言他會怎麼做（「你就是不會去」），再把「我不會停」當籌碼。
#   **角色的狀態一旦能拿來當籌碼，可愛就變成情勒。**
#
# 底線不變：講的每件事都要是真的產品行為。「沒有關閉按鈕」是設計決定，
# 「它沒有手」是它真的做不到的事。讀的人看完之後知道的事情要是對的。
#
# 按鈕與開關的標籤不在這個範圍內，那些仍然是標籤，仍然受 test_copy_style.py 管。
# copy-style: off
WATER_LEAD = ("這是一隻住在螢幕上緣的杯子。它會記時間，時間到就滑下來。"
              "它沒有手，倒水的部分要你來。")
FILL_LEAD = "去裝一壺回來。它會在原地等，其他都不用設定。"
# 彩蛋真的開起來了才這樣寫。開失敗還說「配了首歌」就是介面在說謊。
FILL_LEAD_SONG = "去裝一壺回來，順便配了首歌。它會在原地等，其他都不用設定。"
HOW_BULLETS = ("平常它不在畫面上",
               "時間到，從螢幕上緣滑下來",
               "點它一下，記錄一次補水，它就離開",
               "沒有關閉按鈕")
HOW_SETTINGS = "每日目標與提醒間隔可隨時在設定中調整，從系統匣圖示或紀錄視窗右上角的齒輪進入。"
# copy-style: on
