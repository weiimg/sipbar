...
class Island(QWidget):
    ...
    def _is_late(self, hour=None):
        """現在算不算夜間。範圍是睡前三小時起，到清晨換日為止。

        早上那一端先前綁在「習慣起床時間」（`cfg["day_rollover_hour"]`）上，
        也就是一路延續到那個**宣告值**：起床設 10 點的人，早上 7、8、9 點
        坐在電腦前，間隔仍然被乘上 1.45（實測 86 分），而畫面刻意不解釋
        （理由見 `_status_sub()`），他只看到一個沒有原因的長倒數。

        放慢的理由是「睡前灌水會半夜起來上廁所」（見 `_roll_interval()`）。
        **那個理由在睡前成立，在早上提早起來工作完全不成立。**

        改綁 `settings.DAY_ROLLOVER_HOUR`（早上 5 點）。這裡不另外做「今天
        第一次有活動就算醒了」那種事實訊號，因為那個訊號已經在了：人在睡覺時
        倒數本來就不會走（tick 的閒置門檻擋著），所以 5 點到真正起床之間，
        這個判斷根本不會被讀到。它會被讀到的每一刻，都代表人真的坐在電腦前，
        而那就是醒著。用一個常數換到跟事實訊號一樣的結果，不必新增任何
        跨重啟的狀態——**判斷「人醒了沒」這件事本身才是會出錯的部分。**

        還有一個好處是白撿的：夜間的結束與換日變成同一個時刻，而 `tick()` 的
        換日分支本來就會重擲間隔。所以「昨晚 23:00 擲出的 109 分一路跟到早上」
        不必另外處理，跨過 5 點的每一條路徑（程式開著、關掉重開、睡眠喚醒）
        都會經過那個分支。

        三種寫法要分開。舊的 `hour >= start or hour < end` 在極端設定下會變成
        「整天都是夜間」：早上 6 點才睡的人，起點被推導成 3 點，
        `hour >= 3 or hour < 5` 涵蓋全部 24 小時，提醒從此永遠放慢。
        """
        hour = datetime.now().hour if hour is None else hour
        start = self.cfg["late_night_start_hour"] % 24
        end = settings.DAY_ROLLOVER_HOUR
        if start == end:
            return False                            # 起點與終點重疊＝沒有夜間
        if start > end:
            return hour >= start or hour < end      # 跨午夜，正常的 21 -> 5
        return start <= hour < end                  # 不跨午夜（極端早睡的人）
