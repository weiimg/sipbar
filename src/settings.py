...
# 提醒後人要先把路徑指到沙箱；第二次是**讀過那段註解的人**照樣踩下去，
...
def real_write_violations():
    ...
    為什麼需要這個：Qt 會**吞掉 slot 裡拋出的例外**——它把 traceback 印到
...
def _migrate_data_dir():
    ...
    **只在新資料夾還沒有 config.json 時做一次**，之後永遠不再碰舊的那個。
    ...
    舊資料夾從**現在的 DATA_DIR 的同層**去推，不寫死絕對路徑——測試把 DATA_DIR
...
def target_from_weight(kg, ml_per_drink=None):
    ...
    體重 × 30cc  = 每日**飲品**目標，不含食物水分。這個定義要寫對：
...
def apply_auto_schedule(cfg, events_path):
    ...
    # 深夜起點則**永遠重算**——它是就寢時間的導出值，來源變了它就該跟著變。
